...
// node script that didnt work out
// uses papa parse library (need to have that locally)
...
var app = express();
const file = fs.readFileSync("Responses.csv", {
  encoding: "utf8",
  flag: "r",
});
...
for (i = 1; i < data1.length; i++) {
  for (j = 0; j < data1[i].length; j++) {
    ...
    for(k = 0; k < data1[i][j].length; k++)
     res.push(data1[i][j][k].split(";"))
